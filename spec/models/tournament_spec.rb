...
RSpec.describe Tournament, type: :model do
  ...
  context 'Event' do
    it 'is valid with date into event date range' do
      ...
      tournament = build(:tournament, date: "11/01/2016")
      expect(tournament).to be_valid
    end
    it 'is invalid with date before event date range' do
      event      = create(:event, start_date: "10/01/2016", end_date: "12/01/2016")
      tournament = build(:tournament, date: "9/01/2016")
      tournament.valid?
    ...
    it 'is invalid with date after event date range' do
      event      = create(:event, start_date: "10/01/2016", end_date: "12/01/2016")
      tournament = build(:tournament, date: "13/01/2016")
      tournament.valid?
  ...
  context 'Participant' do
    it 'can be stored if participants count is lower than max participants' do
      ...
      expect(tournament.push(participant)).to be_valid
    end
    it 'cant be stored if participants count is equal or higher than max participants' do
      tournament  = create(:tournament, max_competitors: 0, max_substitutes: 0)
      participant = create(:participant)
      tournament.push(participant).valid?
      expect(tournament.errors[:participant]).to include ("max participants are rearched")
    end

    it 'over minimal age can be inscribed' do
      Timecop.travel Time.parse("2/1/2020")
      tournament  = create(:tournament, min_age: 10)
      player      = create(:player, birthday: "2/1/2000")
      participant = create(:participant, player: player)
      expect(tournament.push(participant)).to be_valid
      Timecop.return
    end
    ...
    it 'under minimal age cant be inscribed' do
      ...
      participant = create(:participant, player: player)
      tournament.push(participant).valid?
      expect(tournament.errors[:participant]).to include ("player under minimal age")
      Timecop.return
    end
  end
